...
<p align="center">
  <b>Современный графический интерфейс (WinUI 3) для автоматического обхода DPI блокировок YouTube, Discord и блокировок Telegram в один клик.</b>
</p>
...
<p align="center">
  <a href="https://github.com/joycecurcirt539-dot/zapret-mirrly-gui/releases/latest">
    <img src="https://img.shields.io/badge/⬇%EF%B8%8F_%D0%A1качать_v1.1.0-ZapretMirrlyGUI.exe-6e40c9?style=for-the-badge&logo=github" alt="Скачать последнюю версию" />
  </a>
</p>
...
* ⚡ **Менеджер пресетов:** Стратегии обхода DPI (FAKE TLS, SIMPLE FAKE, ALT) для YouTube и Discord. Поддержка собственных аргументов.
* ✈️ **Встроенный обход Telegram (C# TgWsProxy):** Высокоскоростной встроенный прокси-сервер на чистом C# (.NET 10) для разблокировки Telegram. Поддерживает пулы веб-сокетов, маскировочный Fake TLS SNI, Cloudflare Workers и быстрое подключение прямо из трея.
...
| Встроенный обход Telegram (C#) | ✅ (Высокоскоростной) | ❌ | ❌ |
...
**Да!** Начиная с версии **v1.1.0**, в Zapret Mirrly GUI полностью встроен высокопроизводительный WebSocket-прокси для обхода блокировок Telegram на территории РФ.

В отличие от оригинального репозитория `tg-ws-proxy` (написанного на Python), наш прокси-сервер переписан на чистом C# (.NET 10) и интегрирован непосредственно в основной процесс GUI. Это дало:
* **Нулевую сетевую задержку**: Реализован пул предпрогретых веб-сокетов (`WsPool`) к официальным датацентрам Telegram. Соединения открыты заранее, сообщения и медиа отправляются мгновенно.
* **Аппаратное шифрование**: Потоковое шифрование MTProto секретов использует криптографические инструкции процессора AES-NI (`Aes.EncryptEcb`).
* **Отсутствие зависимости от Python**: Больше никаких тяжелых EXE-файлов в ресурсах, отсутствие ложных детектов антивирусов на PyInstaller и сниженный на ~41 МБ вес приложения.
* **Удобное подключение**: Добавить прокси в клиент Telegram можно в один клик прямо из системного трея.
