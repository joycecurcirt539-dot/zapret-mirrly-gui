...
2. [Нововведения версии 1.1.2](#нововведения-версии-112)
...
## Нововведения версии 1.1.2

Версия **1.1.2** предлагает масштабное обновление визуального оформления, системы тем, сетевой подсистемы диагностики и параметров автоматизации.

### 1. Полноценная система тем: Светлая, Тёмная и «Чёрный графит» (AMOLED)
Приложение получило полностью переработанный цветовой движок с поддержкой трех базовых визуальных режимов:
* **Чёрный графит (AMOLED):** Бескомпромиссный глубокий 100% pitch black (#000000) фон для OLED и AMOLED дисплеев. Элементы управления и плашки становятся прозрачными, создавая монолитный внешний вид с минимальным энергопотреблением.
* **Светлая тема:** Контрастный, яркий дизайн с подложкой в стиле матового стекла (Apple Light Glass), адаптированный для работы при ярком освещении.
* **Тёмная тема:** Классический глубокий графитовый стиль для комфортного использования в ночное время.

### 2. Оптический стеклянный эффект: Аппаратный Acrylic и Mica (DWM)
Осуществлена глубокая интеграция с Windows Composition API и Desktop Window Manager (DWM):
* Динамическая передача нативного атрибута `DWMWA_USE_IMMERSIVE_DARK_MODE` на уровне системы, что обеспечивает правильную тонировку системных размытий в зависимости от выбранной темы.
* Полный отказ от устаревших слоев `WS_EX_LAYERED`, вызывшавших мерцания и снижение производительности. Аппаратное размытие работает нативно с высокой частотой кадров.

### 3. Универсальный диагностический модуль сетевой подсистемы
Разработан универсальный PowerShell-скрипт диагностики (`test zapret.ps1`):
* Поддерживает работу с любыми версиями и сторонними сборками Zapret без привязки к жестко заданным путям.
* Выполняет пошаговое тестирование сетевой подсистемы: от проверки службы BFE и драйвера WinDivert до анализа подмены DNS, пинга и проверки реального отклика HTTP/HTTPS целевых сервисов.

### 4. Автоматическое обучение списка (Auto Hostlist)
Добавлена поддержка динамического режима `autohostlist.txt`:
* Движок `winws` в фоновом режиме отслеживает сетевые сбои при обращении к ресурсам и автоматически вносит заблокированные домены в список обхода непосредственно во время веб-серфинга.
* Избавляет от необходимости вручную формировать и пополнять текстовые файлы черных списков.

### 5. Селективная фильтрация по сетевым протоколам (IPv4 / IPv6)
Добавлен переключатель режимов сетевого перехвата:
* Возможность жестко ограничить работу обходчика только протоколом IPv4, только IPv6 или использовать совмещенный режим.
* Позволяет исключить задержки и ошибки обхода у провайдеров, не имеющих полноценной поддержки IPv6.

### 6. Усовершенствованный интерактивный системный трей
Трей получил полное визуальное и функциональное обновление:
* Виджет быстрого управления по левому клику и полноценное контекстное меню по правому клику.
* Прозрачность подложки трея адаптирована под выбранную тему оформления, с четким контрастным отображением иконок и текста.
* Быстрое переключение пресетов, запуск диагностики и управление Telegram-прокси в один клик.

### 7. Пользовательские пути и интеллектуальное резервное копирование
* Поддержка выбора произвольных каталогов для Zapret, черных/белых списков и бэкапов.
* Автоматическая система создания резервных копий пользовательских конфигураций перед их изменением.

### 8. Защищенное обновление параметров Telegram-прокси (TgWsProxy)
* Безопасная настройка параметров подключения (хост, порт, FakeTLS домен, размер пула соединений `WsPool`).
* Динамическое применение настроек без сброса активных сессий мессенджера и падения потоков шифрования AES-NI.

### 9. Структурированный раздел настроек и база знаний
* Страница настроек переработана для исключения мерцания при переключении вкладок.
* Добавлены подразделы «Поддержать автора», «Справка» с руководствами по решению сетевых проблем и интерактивная проверка обновлений.

### 10. Модернизация элементов интерфейса и иконографии
* Обновлен сет векторных Fluent-иконок для всех разделов приложения.
* Улучшена визуальная обратная связь для кнопок состояния и элементов управления.
