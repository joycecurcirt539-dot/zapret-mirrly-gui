...
> [!IMPORTANT]
> **Нативный Win32 API стек (GUI v1.1.5):** Начиная с версии v1.1.5, приложение полностью отказалось от запуска внешних PowerShell скриптов и консольного `service.bat` при работе через GUI. Все системные проверки (BFE, WinDivert), опрос служб, диагностика протоколов и настройка TCP Timestamps выполняются нативно в C#. Тестирование DPI-доступа производится с помощью вызова встроенной утилиты `curl.exe` напрямую из асинхронного C# диагностического движка без использования командных оберток.
> 
...
* **Бесшумность (0% мигающих окон):** Забудьте о вылетающих черных консольных окнах `cmd.exe` или `powershell`.
* **Интегрированный диагностический движок:** Тестирование доступности портов и DPI блоков (TLS 1.2 / TLS 1.3) выполняется через вызов встроенной утилиты `curl.exe` напрямую из асинхронного C# диагностического движка с передачей параметров через native-списки аргументов (что защищает от багов с путями и пробелами).
...
После глубокой оптимизации для работы приложения больше не требуются скрипты `service.bat` и внешнее выполнение PowerShell. Минималистичная рабочая структура выглядит так:

```text
zapret/
├── bin/
│   ├── winws.exe                   # Нативный движок обхода DPI
│   ├── curl.exe                    # Утилита для выполнения диагностических HTTP-запросов
...
│   └── test zapret.ps1              # Скрипт диагностики (сохранен для ручного запуска из CLI)
